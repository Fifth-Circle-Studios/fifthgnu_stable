...
pub fn install_bootloader_efi(efidir: PathBuf) {
    ...
    files_eval(
        files::append_file(
            ...
            "GRUB_THEME=\"/usr/share/grub/themes/FifthGNU/theme.txt\"",
        ),
        "enable FifthGNU grub theme",
    );
    exec_eval(
        exec_chroot(
            "grub-mkconfig",
...
pub fn setup_unakite(root: &str, oldroot: &str, efi: bool, efidir: &str, bootdev: &str) {
    ...
    exec_eval(
        exec(
            ...
            vec![
                String::from("-i"),
                String::from("-e"),
                String::from("s/FifthGNU/unakite/g"),
    ...
    exec_eval(
        exec(
            "cp",
            vec![
                String::from("/tmp/ouroboros.json"),
                String::from("/mnt/etc/installSettings.json"),
            ],
        ),
        "Copy ouroboros.json to /etc/installSettings.json in unakite",
    );
